...
- `takeLatest` is used for listening for a particular action. In this case, it will wait for a `LOAD_STRINGS` action. Whenever you dispatch this action, the saga will understand that you want to fetch repos from github's public API by calling `getRepos()`.
